...
// console.log("token")
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL+"api/constructionstatus", {
      method: "POST",
      headers: {
  ...
  export async function getConstructionstatusTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL+"api/constructionstatus"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteConstructionstatusAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL+`api/constructionstatus/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
